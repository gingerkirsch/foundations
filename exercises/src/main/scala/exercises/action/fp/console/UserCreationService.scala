...
import exercises.action.fp.console.UserCreationService.parseLineToBoolean
...
class UserCreationService(console: Console, clock: Clock) {
  ...
  import console.{readLine, writeLine}  // local to the class
  ...
  for {
      ...
      line <- readLine
      dateOfBirth <- parseDateOfBirth(line).onError(_ => printError)
    } yield dateOfBirth
  ...
  val readSubscribeToMailingList: IO[Boolean] =
    for {
      ...
      line <- readLine
      yesNo <- parseLineToBoolean(line).onError(_ =>
