...
case class SearchResult private (flights: List[Flight]) { // we can still call it inside of companion object
...
object SearchResult {
  def apply(flights: List[Flight]): SearchResult = {
    val distinct = flights
      .groupBy(_.flightId)
      .map{ case (flightId, flightSameId) => flightSameId.minBy(_.unitPrice) }
      .toList
